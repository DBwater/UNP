...
int setblocking(int sockfd)
{
    ...
    fcntl(sockfd,F_SETFL,new_flags);
    return old_flags;
}
void addfd(int epollfd,int sockfd,int one_shot){
    epoll_event event;
...
void http_conn::init(int sockfd,const sockaddr_in& addr)
{
    ...
    //用于调试避免出现TIME_WAIT状态；
    //int reuse = 1;
    //setsockopt(m_socket,SQL_SOCKET,SO_REUSEADDR,&reuse,sizeof(reuse));
...
//解析http的请求行，获得请求的方法，目标URL,以及HTTP的版本号
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text){
    //遇到空行，头部字段解析完毕
    if(text[0]=='\0'){
        //如果HTTP有消息体，还需要读取m_content_length字节的消息体，状态机转移到CHECK_STATE_CONTENT状态
    ...
    //处理Connection头部字段
        ...
        printf("unknow headr %s\n",text);
    }
    return NO_REQUEST;
}

//无需解析HTTP请求的消息体，只需要判断他是否被完整读入，可以自行扩展
...
bool http_conn::write(){
    ...
    while(1){
       ...
       if(temp <= -1){
           //如果TCP写缓冲没有空间，则等待下一轮EPOLLOUT事件，虽然在此期间，服务器无法立即接受到同一客户的下一个请求，但是这样可以保持连接的完整性
           if(errno == EAGAIN){
